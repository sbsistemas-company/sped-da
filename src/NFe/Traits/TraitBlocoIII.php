...
trait TraitBlocoIII
{
    ...
    protected function PosIII($pos)
    {
        ...
        $espaco = "        ";
        $pos .= "<left><font.B><b>Codigo  Descricao                 Qtde    UN   V.Unit   V.Total </b><br>";
        ...
        foreach ($this->itens as $item) {
            ...
            $it = (object) $item;
            // Codigo
            $IteCod = str_pad($it->codigo, 8); // Faz o Ite Código ter 8 espaços
            // Nome
            $NomeBR = "";
            if (strlen($it->desc)<=24){
                $IteNome = str_pad($it->desc, 25);
            } else {
                $IteCortado = substr($it->desc,0, 48);
                $IteNome = str_pad(substr($IteCortado,0,24), 26);
                $NomeBR = str_pad(substr($IteCortado, 24, 24), 26);
            }
            // Unidade
            $IteQtd = str_pad(substr($it->qtd,0,8),8);
            $IteUn = str_pad(substr($it->un,0,2),4);

            $IteVunit = str_pad($it->vunit, 9);
            $IteValor = str_pad($it->valor, 8);
            $pos .= "{$IteCod}{$IteNome}{$IteQtd}{$IteUn}{$IteVunit}{$IteValor}<br>";
            if ($NomeBR != ""){
                $pos .="$espaco{$NomeBR}<br>";

            }

        }
        $pos .= "</font.B><line></left>";
